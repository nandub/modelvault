use modelvault::{diff::{diff_models, diff_report}, manifest::{ArtifactManifest,ChunkRef,TensorManifest}};
...
#[test]
fn model_diff_report_includes_artifact_metadata_and_reuse_totals() { let a=manifest("a","111"); let b=manifest("b","111"); let report=diff_report(&a,&b); assert_eq!(report.version,1); assert_eq!(report.left.artifact_id,"a"); assert_eq!(report.right.artifact_id,"b"); assert_eq!(report.reused_right_bytes,4); let json=serde_json::to_value(report).unwrap(); assert_eq!(json["diff"]["unchanged"],1); }
