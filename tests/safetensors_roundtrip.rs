...
use modelvault::{
    artifact::{
        add_safetensors_artifact, materialize, materialize_selected_safetensors,
        resolve_selected_tensor_names,
    },
    cas::LocalCas,
...
fn safetensors_artifact_round_trips_and_records_tensor_boundaries() {
    ...
    assert!(added
        ...
        .chunks
        .iter()
        .any(|c| c.tensor.as_deref() == Some("layer.a")));
    assert!(added
        .manifest
        .chunks
        .iter()
        .any(|c| c.tensor.as_deref() == Some("layer.b")));
...
fn selected_tensors_materialize_as_a_valid_derived_safetensors_file() {
    ...
    let added = add_safetensors_artifact(&source, &cas, 1024).unwrap();
    let result =
        materialize_selected_safetensors(&added.manifest, &cas, &["layer.b".to_string()], &output)
            .unwrap();
...
fn tensor_prefix_selectors_are_resolved_in_manifest_order() {
    ...
    tensors.insert(
        "encoder.a",
        TensorView::new(Dtype::U8, vec![32], &a).unwrap(),
    );
    tensors.insert(
        "encoder.b",
        TensorView::new(Dtype::U8, vec![32], &b).unwrap(),
    );
    tensors.insert(
        "head.weight",
        TensorView::new(Dtype::U8, vec![32], &c).unwrap(),
    );
    serialize_to_file(tensors, None, &source).unwrap();
    let cas = LocalCas::open(temp.path().join(".modelvault")).unwrap();
    let added = add_safetensors_artifact(&source, &cas, 32).unwrap();

    let selected = resolve_selected_tensor_names(
        &added.manifest,
        &["head.weight".into()],
        &["encoder.".into()],
    )
...
fn extract_tensors_can_import_a_derived_artifact_and_record_lineage() {
    ...
    let source_pointer =
        ArtifactPointer::load(repo.path().join("models/source.safetensors.mvptr")).unwrap();
    let derived_pointer = ArtifactPointer::load(
        repo.path()
            .join("models/derived/selected.safetensors.mvptr"),
    )
    .unwrap();
    let (_, derived) = derived_pointer.resolve_manifest(repo.path()).unwrap();
    assert_eq!(derived.lineage.len(), 1);
    assert_eq!(
        derived.lineage[0].parent_artifact_id,
