...
use crate::{cas::{LocalCas, ObjectId}, manifest::ArtifactManifest};
...
pub fn analyze_delta_potential(
    ...
    let left_by_range = left.chunks.iter()
    ...
    for right_chunk in &right.chunks {
        ...
        let left_bytes = cas.read(&left_id).with_context(|| format!("failed to read left object {left_id}"))?;
        let right_bytes = cas.read(&right_id).with_context(|| format!("failed to read right object {right_id}"))?;
        if left_bytes.len() != right_bytes.len() {
            report.incomparable_bytes = report.incomparable_bytes.saturating_add(right_chunk.size);
            continue;
        }
        report.comparable_chunks += 1;
        let delta = left_bytes.iter().zip(&right_bytes).map(|(a, b)| a ^ b).collect::<Vec<_>>();
        ...
        #[cfg(feature = "compression")]
        {
            ...
            report.full_compressed_bytes = report.full_compressed_bytes.saturating_add(full.len() as u64);
            report.delta_compressed_bytes = report.delta_compressed_bytes.saturating_add(delta_encoded.len() as u64);
        }
    ...
    report.potential_savings_bytes = report.full_compressed_bytes.saturating_sub(report.delta_compressed_bytes);
...
pub fn optimize_delta_storage(
    ...
    for right_chunk in &right.chunks {
        let Some(left_chunk) = left_by_range.get(&(right_chunk.offset, right_chunk.size)).copied() else {
            continue;
        };
        if left_chunk.object == right_chunk.object || !seen_targets.insert(right_chunk.object.clone()) {
            continue;
        }
        report.candidates += 1;
        let target = ObjectId::parse(&right_chunk.object)?;
        let base = ObjectId::parse(&left_chunk.object)?;
        let result = cas.optimize_object_as_delta(
            &target,
            &base,
            zstd_level,
            min_savings_pct,
        ...
        if result.stored {
            report.stored += 1;
            report.full_physical_bytes = report.full_physical_bytes.saturating_add(result.full_physical_bytes);
            report.delta_physical_bytes = report.delta_physical_bytes.saturating_add(result.delta_physical_bytes);
