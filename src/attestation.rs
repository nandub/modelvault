...
fn key_bytes(path: &Path, expected_len: usize, kind: &str) -> anyhow::Result<Vec<u8>> {
    let text = fs::read_to_string(path)
        .with_context(|| format!("failed to read {kind} key {}", path.display()))?;
    let bytes = STANDARD
        .decode(text.trim())
        .with_context(|| format!("{kind} key must be base64"))?;
    ensure!(
        bytes.len() == expected_len,
        "{kind} key must decode to {expected_len} bytes"
    );
    Ok(bytes)
}
...
pub fn manifest_digest(manifest: &ArtifactManifest) -> anyhow::Result<String> {
    Ok(blake3::hash(&manifest_payload(manifest)?)
        .to_hex()
        .to_string())
}
...
pub fn default_attestation_path(store: &Path, artifact_id: &str) -> PathBuf {
    store
        .join("attestations")
        .join(format!("{artifact_id}.ed25519.json"))
}
...
fn write_new_key(path: &Path, bytes: &[u8], kind: &str) -> anyhow::Result<()> {
    ...
    let mut file = fs::OpenOptions::new()
        ...
        .with_context(|| {
            format!(
                "failed to create {kind} key {}; existing keys are never overwritten",
                path.display()
            )
        })?;
...
pub fn attest_manifest(
    ...
    key_id: &str,
) -> anyhow::Result<ManifestAttestation> {
    ensure!(
        !key_id.trim().is_empty() && key_id.len() <= 128,
        "--key-id must be between 1 and 128 bytes"
    );
...
pub fn verify_attestation(
    ...
    let attestation: ManifestAttestation = serde_json::from_slice(&fs::read(attestation_path)?)?;
        ...
        "attestation artifact ID does not match manifest"
    );
    ensure!(
        attestation.manifest_blake3 == manifest_digest(manifest)?,
        "attestation manifest digest does not match manifest"
    );
    ...
    let signature_bytes = STANDARD
        ...
        .context("attestation signature must be base64")?;
    let signature = Signature::from_slice(&signature_bytes).context("invalid Ed25519 signature")?;
    key.verify(attestation.manifest_blake3.as_bytes(), &signature)
