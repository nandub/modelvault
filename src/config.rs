...
impl RemoteDefinition {
    ...
    pub fn s3(
        ...
        ensure!(max_attempts > 0, "S3 max attempts must be greater than zero");
        Ok(Self {
            kind: "s3".to_string(),
            path: None,
            bucket: Some(bucket),
            prefix: prefix.map(|p| normalize_prefix(&p)).filter(|p| !p.is_empty()),
            region,
            endpoint,
            force_path_style,
            profile,
...
impl ModelVaultConfig {
    ...
    fn add_remote(&mut self, name: &str, remote: RemoteDefinition) -> anyhow::Result<()> {
        ...
        ensure!(!self.remotes.contains_key(name), "remote '{name}' already exists");
        self.remotes.insert(name.to_string(), remote);
        Ok(())
    }

    pub fn remove_remote(&mut self, name: &str) -> anyhow::Result<()> {
        ensure!(self.remotes.remove(name).is_some(), "remote '{name}' does not exist");
    ...
    pub fn set_default(&mut self, name: &str) -> anyhow::Result<()> {
        ensure!(self.remotes.contains_key(name), "remote '{name}' does not exist");
    ...
    pub fn resolve(&self, selector: Option<&str>) -> anyhow::Result<(String, RemoteDefinition)> {
        let name = selector
            ...
            .ok_or_else(|| anyhow::anyhow!("no remote specified and no default remote is configured"))?;
        let remote = self
            .remotes
            .get(&name)
            .cloned()
