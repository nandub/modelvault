...
# Review the dry-run, then delete only unreachable objects:
...
reads and BLAKE3-verifies referenced object bodies. With the optional `s3`
feature, the same commands audit the configured S3/MinIO prefix using bounded,
paginated listing and only consider canonical ModelVault manifest/object keys.

`remote storage` reports remote logical/physical use and orphan accounting.
`remote gc` is dry-run by default and uses logical manifest reachability,
including delta-base dependencies; only `--prune` permits deletion.
...
creates a unique bucket, deep-verifies a push plus `remote fsck --deep` and
`remote storage`, injects an unreachable test object, verifies dry-run `remote
gc` reports it, prunes it, and runs a post-prune deep audit. It then creates a
clean Git clone that deep-verifies a pull, checkout, and `fsck --deep`. It removes the unique
